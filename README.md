...
Готовый проект для GitHub и Render: мобильная клавиатура посетителя и открытая админка с обновлением номера в реальном времени.
...
- `/admin` — открытый чёрный экран оператора без пароля.
- `/health` — проверка состояния Render-сервиса.

Нажатие на синюю кнопку выбора линии в левом верхнем углу основной страницы сразу открывает `/admin` на том же сайте.
